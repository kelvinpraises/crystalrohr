...
export class Colour {
   ...
   * @param {[Number]} z
   */
  static xyz2rgba(x: number, y: number, z: number) {
    let varX = x / 100;
    let varY = y / 100;
    let varZ = z / 100;
  ...
  static deltaE00(
    ...
    interface NewMathInterface extends Math {
      rad2deg: any;
      deg2rad: any;
    }

    const NewMath = Math as NewMathInterface;
    // Utility functions added to Math Object

    NewMath["rad2deg"] = function (rad: number) {
      return (360 * rad) / (2 * NewMath.PI);
    };
    NewMath["deg2rad"] = function (deg: number) {
      return (2 * NewMath.PI * deg) / 360;
    };
    // Start Equation
    // Equation exist on the following URL http://www.brucelindbloom.com/index.html?Eqn_DeltaE_CIE2000.html
    const avgL = (l1 + l2) / 2;
    const c1 = NewMath.sqrt(NewMath.pow(a1, 2) + NewMath.pow(b1, 2));
    const c2 = NewMath.sqrt(NewMath.pow(a2, 2) + NewMath.pow(b2, 2));
    const avgC = (c1 + c2) / 2;
    const g =
      (1 -
        NewMath.sqrt(
          NewMath.pow(avgC, 7) / (NewMath.pow(avgC, 7) + NewMath.pow(25, 7))
        )) /
      2;
    ...
    const c1p = NewMath.sqrt(NewMath.pow(a1p, 2) + NewMath.pow(b1, 2));
    const c2p = NewMath.sqrt(NewMath.pow(a2p, 2) + NewMath.pow(b2, 2));

    const avgCp = (c1p + c2p) / 2;

    let h1p = NewMath["rad2deg"](NewMath.atan2(b1, a1p));
    if (h1p < 0) {
      h1p = h1p + 360;
    }

    let h2p = NewMath["rad2deg"](NewMath.atan2(b2, a2p));
    if (h2p < 0) {
      h2p = h2p + 360;
    }

    const avghp =
      NewMath.abs(h1p - h2p) > 180 ? (h1p + h2p + 360) / 2 : (h1p + h2p) / 2;

    const t =
      1 -
      0.17 * NewMath.cos(NewMath["deg2rad"](avghp - 30)) +
      0.24 * NewMath.cos(NewMath["deg2rad"](2 * avghp)) +
      0.32 * NewMath.cos(NewMath["deg2rad"](3 * avghp + 6)) -
      0.2 * NewMath.cos(NewMath["deg2rad"](4 * avghp - 63));

    let deltahp = h2p - h1p;
    if (NewMath.abs(deltahp) > 180) {
      if (h2p <= h1p) {
        deltahp += 360;
      } else {
        deltahp -= 360;
    ...
    deltahp =
      2 *
      NewMath.sqrt(c1p * c2p) *
      NewMath.sin(NewMath["deg2rad"](deltahp) / 2);

    const sl =
      1 +
      (0.015 * NewMath.pow(avgL - 50, 2)) /
        NewMath.sqrt(20 + NewMath.pow(avgL - 50, 2));
    const sc = 1 + 0.045 * avgCp;
    const sh = 1 + 0.015 * avgCp * t;

    const deltaro = 30 * NewMath.exp(-NewMath.pow((avghp - 275) / 25, 2));
    const rc =
      2 *
      NewMath.sqrt(
        NewMath.pow(avgCp, 7) / (NewMath.pow(avgCp, 7) + NewMath.pow(25, 7))
      );
    const rt = -rc * NewMath.sin(2 * NewMath["deg2rad"](deltaro));

    const kl = 1;
    const kc = 1;
    const kh = 1;

    const deltaE = NewMath.sqrt(
      NewMath.pow(deltalp / (kl * sl), 2) +
        NewMath.pow(deltacp / (kc * sc), 2) +
        NewMath.pow(deltahp / (kh * sh), 2) +
